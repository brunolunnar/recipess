...
// Defina um tipo para o retorno do FaunaDB
interface FaunaDBResponse {
  data: any; // Atualize isso com o formato de dados real retornado pelo FaunaDB
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "DELETE") {
    try {
      ...
      res.status(204).end(); // Resposta 204 indica que a exclusão foi bem-sucedida
    } catch (error: any) {
